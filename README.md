...
3. **실거래 고객 리뷰(생생 거래 후기) & 실제 매물 정보 직접 등록 시스템**
   - **실매물 연동 리뷰 카드**: 거래 매물명, 층수/호실, 전용면적, 거래금액, 층고/전력 스펙 및 고객사 별점(★ 5.0) 결합 표시
   - **홈페이지 즉시 등록 모달**: 방문자 및 중개사가 `[✍️ 거래 후기 직접 등록하기]`를 눌러 실제 매물 스펙과 후기를 실시간 등록 가능
   - **관리자 3대 통합 탭 (`/admin.html`)**:
     * ⭐ **고객 리뷰 & 실매물 관리**: 실제 거래 후기 등록, 수정, 삭제
     * 🏢 **추천 매물 직접 관리**: 홈페이지 실시간 매물(`properties.json`) 직접 등록 및 관리
     * 📰 **부동산 소식·뉴스 관리**: 개발호재, 공장/창고 트렌드 칼럼 관리

4. **Vercel 서버리스 + GitHub JSON 영구 보존 데이터베이스**
   - 유료 외부 DB 없이, GitHub 저장소의 `data/reviews.json`, `data/properties.json`, `data/board.json` 파일을 데이터베이스로 사용
   - 어드민 페이지(`/admin.html`) 및 홈페이지 모달에서 데이터 변경 시 Vercel 서버리스 함수(`api/reviews.js`, `api/properties.js`, `api/board.js`) 및 로컬 Python 서버가 자동 반영 및 영구 보존
